...
`appBoilerplate` is designed to make life easy by providing a basic framework
with which to kickstart Linked Data Web app projects. It contains a best-practice
directory structure to ensure code reusability and maximum scalability.
This boilerplate app also comes prepackaged with the most popular design frameworks
...
`appBoilerplate` uses [Grunt](http://gruntjs.org) as its build system, so
...
There are many Bower packages used by `appBoilerplate`, like Twitter Bootstrap
...
Technically, `appBoilerplate` is now ready to go.
...
`appBoilerplate`. What follows in this section is a quick introduction to the
...
`appBoilerplate` also includes [Live Reload](http://livereload.com/), so you no
...
default with `appBoilerplate`) you need to tell Live Reload to allow it. Go to
